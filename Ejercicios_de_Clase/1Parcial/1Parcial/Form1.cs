...
namespace _1Parcial
{
    public partial class Form1 : Form
    {
        ...
        { 
            ...
            dataGridView2.DataSource = null;dataGridView2.DataSource = (from px in p select new { Id = px.Id, Descripción = px.Descripcion, Vto = px.FechaVto, Costo = px.Costo, Tipo = px is ProductoA ? "Producto A" : "Producto B" }).ToList();
            var _aux = from px in p select px.Costo;
            decimal _total = 0;
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                p.Remove((dataGridView1.SelectedRows[0].DataBoundItem) as Producto);
                Mostrar(dataGridView1, p);
            }
            catch (Exception ex) { MessageBox.Show(ex.Message); }
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                foreach(Producto _p in p) { _p.ChequeoVencidos(); }
            }
            catch (Exception ex) { MessageBox.Show(ex.Message); }
